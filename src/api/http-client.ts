...
function mapAjaxRequestJson(request?: PartAjaxRequest) {
    const token = Utils.getValueLocalStorage("token");
    const key = Utils.getValueLocalStorage("key")
    const mapHeaders = request?.headers
        ? ({ ...request.headers } as HeadersAjax)
...
function commonApiCallJson(
    method: HttpMethod,
    param: Param,
    isGetHeader = false
): Observable<unknown> {
    const { url, data, headers } = param;
    const newHeaders = mapAjaxRequestJson(headers);
    const body = data;
    return ajax({ url, method, body, ...newHeaders }).pipe(
        map((res: AjaxResponse) =>
            !isGetHeader ? mapResponse(res) : mapResponseHeader(res)
...
export default class HttpClient {
    ...
    static post(
        ...
        return commonApiCallJson("POST", { url, data, headers }, isGetHeader);
    }
    ...
    static put(
        ...
        return commonApiCallJson("PUT", { url, data, headers });
    }
    // static upload(
    //     url: string,
    //     data: unknown,
